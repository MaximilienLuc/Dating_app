...
**Historical results:** the committed numbers predate the updated group dataset and reduced logit feature list. Do not use them for the final presentation. The source notebook now refuses stale results.

Rémi's contribution to HEC Match. Preliminary results for the group's v0 logit and XGBoost, using the existing data and session split. TabICL remains to be added when Alex's implementation is ready.
...
## Per-model feature contract

The logit uses `features_logit` when present, otherwise the shared `features` list for backwards compatibility. XGBoost always uses `features`. Selection is applied to reference fits, every bootstrap fit, predictions, coefficient labels and coefficient scaling. The report records the exact lists and the source of the logit list. The fallback does not claim to implement the reduced-preference model.

Stability dependencies now include the group’s pinned `requirements.txt`, rather than independently selecting newer estimator versions.
